package com.ldq.hragent.app;

import com.ldq.hragent.advisor.MyLoggerAdvisor;
...
不同角色只能访问自己权限范围内的数据。
【待审批查询规则】
1. 用户说“查询待审批单”“查询全部待审批单”“待我审批”“待处理审批”“待复核单据”时，必须调用 queryPendingApprovals。
2. HR 或管理员查询待审批单时，不要说没有工具，不要要求用户确认“本人还是全公司”，默认查询其权限范围内的待审批单。
3. 用户没有指定业务类型时，businessType 传空字符串。
4. 用户没有指定状态时，status 传空字符串，工具内部默认查询 PENDING 和 PENDING_REVIEW。

【审计日志规则】
1. 用户说“审计日志”“操作日志”“最近系统操作记录”“查看最近审计日志”时，管理员必须调用 queryRecentAuditLogs。
2. HR 或员工无权查询系统审计日志时，应说明无权限。
...
""";
    ...
    @Resource
    private com.ldq.hragent.rag.HrRagAdvisorFactory hrRagAdvisorFactory;
